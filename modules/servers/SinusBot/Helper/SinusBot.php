...
class SinusBot {
	...
	public function login( $username, $password ) {
		$login = $this->request( '/bot/login', 'POST', json_encode( array(
			'username' => $username,
			'password' => $password,
			'botId'    => $this->botUUID
		) ) );
		if ( $login != null AND isset( $login['token'] ) ) {
			$this->wiToken = $login['token'];
		}
	...
	public function deletePlaylistTracks( $playlistUUID ) {
		...
		return $this->request( '/bot/bulk/playlist/' . $playlistUUID . '/files', 'POST', json_encode( array(
			"op"    => "delete",
	...
	public function say( $text, $locale, $instanceUUID = null ) {
		...
		return $this->request( '/bot/i/' . $instanceUUID . '/say', 'POST', json_encode( array(
			"text"   => $text,
			"locale" => $locale
		) ) );
	}
	...
	public function editTrack( $trackUUID, $title, $artist = "", $album = "" ) {
		return $this->request( '/bot/files/' . $trackUUID, 'PATCH', json_encode( array(
			"displayTitle" => $title,
			"title"        => $title,
			"artist"       => $artist,
			"album"        => $album
	...
	public function addURL( $url, $title, $parent = "" ) {
		return $this->request( '/bot/url', 'POST', json_encode( array(
			"url"    => $url,
			"title"  => $title,
			"parent" => $parent
		) ) );
	...
	public function addFolder( $folderName = "Folder", $parent = "" ) {
		return $this->request( '/bot/folders', 'POST', json_encode( array(
			"name"   => $folderName,
			"parent" => $parent
		) ) );
	}
	...
	public function renameFolder( $folderName, $folderUUID ) {
		return $this->request( '/bot/files/' . $folderUUID, 'PATCH', json_encode( array(
			"uuid"  => $folderUUID,
			"type"  => "folder",
			"title" => $folderName
		) ) );
	...
	public function addUser( $username, $password, $privileges = 0 ) {
		return $this->request( '/bot/users', 'POST', json_encode( array(
			'username'   => $username,
			'password'   => $password,
	...
	public function createInstance( $nickname = "TS3index.com MusicBot", $backend = "ts3" ) {
		return $this->request( '/bot/instances', 'POST', json_encode( array(
			"backend" => $backend,
			"nick"    => $nickname
		) ) );
	}
